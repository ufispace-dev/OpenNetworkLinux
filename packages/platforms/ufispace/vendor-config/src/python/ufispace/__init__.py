...
class OnlPlatformUfiSpace(OnlPlatformBase):
    ...
    LEVEL_INFO = 1
    LEVEL_ERR = 2
    PATH_PLT_CFG = "/lib/platform-config/{}/onl/{}"
    PATH_PLTM_CFG = "/lib/platform-config/current/onl"
    PATH_SYS_I2C_DEV_ATTR = "/sys/bus/i2c/devices/{}-{:0>4x}/{}"
    PATH_SYS_GPIO =  "/sys/bus/platform/devices/x86_64_ufispace_gpio/gpio_function"
    PATH_S3IP_CFG = PATH_PLTM_CFG + "/s3ip_config.yml"
    ...
    try:
        with open(PATH_PLT_CFG.format(PLATFORM, "platform.yml"), 'r') as _f:
            _config = yaml.safe_load(_f)
            for _key, _val in _config.items():
                locals()[_key] = _val
            if PATH_LPC:
                PATH_LPC_GRP_BSP = "{}/bsp".format(PATH_LPC)
                PATH_LPC_GRP_CPLD = "{}/mb_cpld".format(PATH_LPC)
                PATH_MUX_RESET_ALL = "{}/mux_reset_all".format(PATH_LPC_GRP_CPLD)
                PATH_BOARD_HW_ID = "{}/board_hw_id".format(PATH_LPC_GRP_CPLD)
                PATH_BOARD_DEPH_ID = "{}/board_deph_id".format(PATH_LPC_GRP_CPLD)
                PATH_BOARD_BUILD_ID = "{}/board_build_id".format(PATH_LPC_GRP_CPLD)
                PATH_I2C_STUCK_STATUS = "{}/i2c_stuck".format(PATH_LPC_GRP_BSP)
        if '_config' in locals(): del locals()['_config']
        if '_key' in locals(): del locals()['_key']
        if '_val' in locals(): del locals()['_val']
        if '_f' in locals(): del locals()['_f']
