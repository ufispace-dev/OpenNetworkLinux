...
bmc_info_t bmc_cache[] =
{
    ...
    [BMC_ATTR_ID_TEMP_ENV_MAC0]      = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_ENV_MAC0     , BMC_DATA_FLOAT , 0},
    [BMC_ATTR_ID_TEMP_MAC0]          = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_MAC0         , BMC_DATA_FLOAT , 0},
    [BMC_ATTR_ID_PSU0_TEMP]          = {HW_PLAT_ALL, BMC_ATTR_NAME_PSU0_TEMP         , BMC_DATA_FLOAT , 0},
    [BMC_ATTR_ID_PSU1_TEMP]          = {HW_PLAT_ALL, BMC_ATTR_NAME_PSU1_TEMP         , BMC_DATA_FLOAT , 0},
...
void check_and_do_i2c_mux_reset(int port)
{
    ...
    if(access(I2C_STUCK_CHECK_SYSFS, F_OK) != -1 ) {

        snprintf(cmd_buf, sizeof(cmd_buf), "cat %s > /dev/null 2>&1", I2C_STUCK_CHECK_SYSFS);
        ret = system(cmd_buf);
...
int get_cpu_hw_rev_id(int *rev_id, int *dev_phase, int *build_id)
{
    ...
    // ONLP_TRY(read_file_hex(rev_id, "/sys/devices/platform/x86_64_ufispace_s9600_102xc_lpc/ec/cpu_rev_hw_rev"));
    // ONLP_TRY(read_file_hex(dev_phase, "/sys/devices/platform/x86_64_ufispace_s9600_102xc_lpc/ec/cpu_rev_dev_phase"));
    // ONLP_TRY(read_file_hex(build_id, "/sys/devices/platform/x86_64_ufispace_s9600_102xc_lpc/ec/cpu_rev_build_id"));
...
int get_thermal_thld(int thermal_local_id,  temp_thld_t *temp_thld) {
    ...
    switch(thermal_local_id) {
        case ONLP_THERMAL_CPU_PECI:
            temp_thld->warning = THERMAL_TEMP_CPU_PECI_WARNING;
            temp_thld->error = THERMAL_TEMP_CPU_PECI_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_CPU_PECI_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV_CPU:
            temp_thld->warning = THERMAL_TEMP_ENV_CPU_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV_CPU_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV_CPU_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV0:
            temp_thld->warning = THERMAL_TEMP_ENV0_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV0_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV0_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV1:
            temp_thld->warning = THERMAL_TEMP_ENV1_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV1_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV1_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV_MAC0:
            temp_thld->warning = THERMAL_TEMP_ENV_MAC0_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV_MAC0_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV_MAC0_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_MAC0:
            temp_thld->warning = THERMAL_TEMP_MAC0_WARNING;
            temp_thld->error = THERMAL_TEMP_MAC0_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_MAC0_SHUTDOWN;
            return ONLP_STATUS_OK;
