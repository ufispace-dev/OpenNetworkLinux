...
SYSFS_CPLD3="${SYSFS_DEV}/1-0032"
SYSFS_CPLD4="${SYSFS_DEV}/1-0033"
SYSFS_CPLD5="${SYSFS_DEV}/1-0034"
SYSFS_LPC="/sys/bus/platform/devices/x86_64_ufispace_s9600_102xc_lpc"
...
function _check_bsp_init {
    ...
    if [ -f "/sys/devices/platform/x86_64_ufispace_s9600_102xc_lpc/bsp/bsp_version" ]; then
        BSP_INIT_FLAG=1
    else
        BSP_INIT_FLAG=0
    fi
...
function _show_board_info {
    ...
    model_id_array=($((2#00011100)))
    model_name_array=("S9600-102XC")
    ...
    # MODEL ID D[0:5]
    model_id=$(((model_id & 2#00111111) >> 0))
    ...
    # Board ID definition
    brd_id_def_id=$(((ext_byte & 2#11000000) >> 6))
    brd_id_def=${brd_id_array[${brd_id_def_id}]}
    ...
    _echo "[BOARD ID Definition      ]: ${brd_id_def}"
}
    ...
    if [[ $MODEL_NAME == "${PLAT}" ]]; then
        ...
        if [ ${ret} -eq ${TRUE} ]; then
            ...
            if _check_i2c_device ${mux_i2c_bus} "0x33"; then
                ...
                mb_cpld4_build=$(eval "i2cget -y -f ${mux_i2c_bus} 0x31 0x4 ${LOG_REDIRECT}")
                _printf "[MB CPLD4 Version]: %d.%02d.%03d\n" $(( (mb_cpld4_ver & 2#11000000) >> 6)) $(( mb_cpld4_ver & 2#00111111 )) $((mb_cpld4_build))
            fi

            if _check_i2c_device ${mux_i2c_bus} "0x34"; then
                mb_cpld5_ver=$(eval "i2cget -y -f ${mux_i2c_bus} 0x34 0x2 ${LOG_REDIRECT}")
                mb_cpld5_build=$(eval "i2cget -y -f ${mux_i2c_bus} 0x31 0x4 ${LOG_REDIRECT}")
...
function _cpld_version_sysfs {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        # CPU CPLD
        if _check_filepath "${SYSFS_LPC}/cpu_cpld/cpu_cpld_version"; then
            cpu_cpld_ver=$(eval "cat ${SYSFS_LPC}/cpu_cpld/cpu_cpld_version ${LOG_REDIRECT}")
        fi

        if _check_filepath "${SYSFS_LPC}/cpu_cpld/cpu_cpld_build"; then
            cpu_cpld_build=$(eval "cat ${SYSFS_LPC}/cpu_cpld/cpu_cpld_version ${LOG_REDIRECT}")
        fi
        
        if _check_filepath "${SYSFS_LPC}/cpu_cpld/cpu_cpld_version_h"; then
            cpu_cpld_ver_h=$(eval "cat ${SYSFS_LPC}/cpu_cpld/cpu_cpld_version_h ${LOG_REDIRECT}")
        fi
...
function _show_i2c_tree_bus_mux_i2c {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        if [ "$ret" == "0" ]; then
            ...
            ## (9548_CPLD_SFP_QSFP)-0x75
            _show_i2c_mux_devices "${bus}" "${chip_addr1}" "8" "9548_CPLD_SFP_QSFP-${chip_addr1}"
            local chip_addr2_array=("" "" "" "0x76" "0x76" "0x76" "0x76" "0x76")
            local mux_name_array=("" "" "" \
                                  "9548_CHILD_QSFP_0_5" "9548_CHILD_SFP_64_71" \
                                  "9548_CHILD_SFP_72_79" "9548_CHILD_SFP_80_87" "9548_CHILD_SFP_88_95")
        ...
        if [ "$ret" == "0" ]; then
            ...
            ## (9548_MGMT)-0x73
            _show_i2c_mux_devices "${bus}" "${chip_addr1}" "8" "9548_MGMT-${chip_addr1}"
        fi
        ...
        if [ "$ret" == "0" ]; then
            ...
            ## (9548_SFP)-0x72
            _show_i2c_mux_devices "${bus}" "${chip_addr1}" "8" "9548_SFP-${chip_addr1}"
            local chip_addr2_array=("0x76" "0x76" "0x76" "0x76" "0x76" "0x76" "0x76" "0x76")
            local mux_name_array=("9548_CHILD_SFP_0_7" "9548_CHILD_SFP_8_15" "9548_CHILD_SFP_16_23" \
                                  "9548_CHILD_SFP_24_31" "9548_CHILD_SFP_32_39" \
                                  "9548_CHILD_SFP_40_47" "9548_CHILD_SFP_48_55" "9548_CHILD_SFP_56_63")
...
function _show_i2c_device_info {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        pca954x_device_bus=("${i801_bus}" "${i801_bus}" "${i801_bus}")
    else
        _echo "Unknown MODEL_NAME (${MODEL_NAME}), exit!!!"
        exit 1
    fi
...
function _show_port_status_sysfs {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        sys_port_array=("${SYSFS_CPLD2}/cpld_sfp_port_0_7_pres"               #0
                         "${SYSFS_CPLD2}/cpld_sfp_port_8_15_pres"             #1
                         "${SYSFS_CPLD3}/cpld_sfp_port_16_23_pres"            #2
                         "${SYSFS_CPLD3}/cpld_sfp_port_24_31_pres"            #3
                         "${SYSFS_CPLD4}/cpld_sfp_port_32_39_pres"            #4
                         "${SYSFS_CPLD4}/cpld_sfp_port_40_43_pres"            #5
                         "${SYSFS_CPLD5}/cpld_sfp_port_44_47_pres"            #6
                         "${SYSFS_CPLD2}/cpld_sfp_port_48_55_pres"            #7
                         "${SYSFS_CPLD2}/cpld_sfp_port_56_63_pres"            #8
                         "${SYSFS_CPLD3}/cpld_sfp_port_64_71_pres"            #9
                         "${SYSFS_CPLD3}/cpld_sfp_port_72_79_pres"            #10
                         "${SYSFS_CPLD4}/cpld_sfp_port_80_87_pres"            #11
                         "${SYSFS_CPLD4}/cpld_sfp_port_88_91_pres"            #12
                         "${SYSFS_CPLD5}/cpld_sfp_port_92_95_pres"            #13
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_101_pres"          #14
                         "${SYSFS_CPLD2}/cpld_sfp_port_0_7_rx_los"            #15
                         "${SYSFS_CPLD2}/cpld_sfp_port_8_15_rx_los"           #16
                         "${SYSFS_CPLD3}/cpld_sfp_port_16_23_rx_los"          #17
                         "${SYSFS_CPLD3}/cpld_sfp_port_24_31_rx_los"          #18
                         "${SYSFS_CPLD4}/cpld_sfp_port_32_39_rx_los"          #19
                         "${SYSFS_CPLD4}/cpld_sfp_port_40_43_rx_los"          #20
                         "${SYSFS_CPLD5}/cpld_sfp_port_44_47_rx_los"          #21
                         "${SYSFS_CPLD2}/cpld_sfp_port_48_55_rx_los"          #22
                         "${SYSFS_CPLD2}/cpld_sfp_port_56_63_rx_los"          #23
                         "${SYSFS_CPLD3}/cpld_sfp_port_64_71_rx_los"          #24
                         "${SYSFS_CPLD3}/cpld_sfp_port_72_79_rx_los"          #25
                         "${SYSFS_CPLD4}/cpld_sfp_port_80_87_rx_los"          #26
                         "${SYSFS_CPLD4}/cpld_sfp_port_88_91_rx_los"          #27
                         "${SYSFS_CPLD5}/cpld_sfp_port_92_95_rx_los"          #28
                         "${SYSFS_CPLD2}/cpld_sfp_port_0_7_tx_fault"          #29
                         "${SYSFS_CPLD2}/cpld_sfp_port_8_15_tx_fault"         #30
                         "${SYSFS_CPLD3}/cpld_sfp_port_16_23_tx_fault"        #31
                         "${SYSFS_CPLD3}/cpld_sfp_port_24_31_tx_fault"        #32
                         "${SYSFS_CPLD4}/cpld_sfp_port_32_39_tx_fault"        #33
                         "${SYSFS_CPLD4}/cpld_sfp_port_40_43_tx_fault"        #34
                         "${SYSFS_CPLD5}/cpld_sfp_port_44_47_tx_fault"        #35
                         "${SYSFS_CPLD2}/cpld_sfp_port_48_55_tx_fault"        #36
                         "${SYSFS_CPLD2}/cpld_sfp_port_56_63_tx_fault"        #37
                         "${SYSFS_CPLD3}/cpld_sfp_port_64_71_tx_fault"        #38
                         "${SYSFS_CPLD3}/cpld_sfp_port_72_79_tx_fault"        #39
                         "${SYSFS_CPLD4}/cpld_sfp_port_80_87_tx_fault"        #40
                         "${SYSFS_CPLD4}/cpld_sfp_port_88_91_tx_fault"        #41
                         "${SYSFS_CPLD5}/cpld_sfp_port_92_95_tx_fault"        #42
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_101_rst"           #43
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_101_lpmode"        #44
                         "${SYSFS_CPLD2}/cpld_sfp_port_0_7_tx_disable"        #45
                         "${SYSFS_CPLD2}/cpld_sfp_port_8_15_tx_disable"       #46
                         "${SYSFS_CPLD3}/cpld_sfp_port_16_23_tx_disable"      #47
                         "${SYSFS_CPLD3}/cpld_sfp_port_24_31_tx_disable"      #48
                         "${SYSFS_CPLD4}/cpld_sfp_port_32_39_tx_disable"      #49
                         "${SYSFS_CPLD4}/cpld_sfp_port_40_43_tx_disable"      #50
                         "${SYSFS_CPLD5}/cpld_sfp_port_44_47_tx_disable"      #51
                         "${SYSFS_CPLD2}/cpld_sfp_port_48_55_tx_disable"      #52
                         "${SYSFS_CPLD2}/cpld_sfp_port_56_63_tx_disable"      #53
                         "${SYSFS_CPLD3}/cpld_sfp_port_64_71_tx_disable"      #54
                         "${SYSFS_CPLD3}/cpld_sfp_port_72_79_tx_disable"      #55
                         "${SYSFS_CPLD4}/cpld_sfp_port_80_87_tx_disable"      #56
                         "${SYSFS_CPLD4}/cpld_sfp_port_88_91_tx_disable"      #57
                         "${SYSFS_CPLD5}/cpld_sfp_port_92_95_tx_disable"      #58
                         "${SYSFS_CPLD1}/cpld_mgmt_sfp_port_status"           #59
                         "${SYSFS_CPLD1}/cpld_mgmt_sfp_port_conf"             #60
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_101_intr")         #61
        ...
        #   88       89       90       91       92       93       94       95
            ${SFP}   ${SFP}   ${SFP}   ${SFP}   ${SFP}   ${SFP}   ${SFP}   ${SFP}
        #   96       97       98       99       100      101      102      103
            ${QSFP}  ${QSFP}  ${QSFP}  ${QSFP}  ${QSFP}  ${QSFP}  ${MGMT}  ${MGMT}
        )
...
function _show_system_led_sysfs {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        local sysfs_attr=("cpld_sys_led_ctrl_1"
                          "cpld_sys_led_ctrl_2"
                          "cpld_sys_led_ctrl_3")
        ...
        local led_sysfs_idx=(2       0       0     1         1       2      )
        local color=(        4       0       4     0         4       -1     )
        local blink=(        6       2       6     2         6       2      )
        local onoff=(        7       3       7     3         7       3      )
        local freq=(         5       1       5     1         5       1      )
