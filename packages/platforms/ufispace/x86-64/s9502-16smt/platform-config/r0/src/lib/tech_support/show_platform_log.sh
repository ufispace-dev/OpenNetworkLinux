...
function _show_system_led_sysfs {
    ...
    if [ "${MODEL_NAME}" == "S9502" ]; then
        front_led_clear=`cat /sys/devices/platform/x86_64_ufispace_s9502_16smt_lpc/mb_cpld/led_clear`
        ret=$?
        if [ $ret -eq 0 ]; then
        ...
        front_led_ctrl=`cat /sys/devices/platform/x86_64_ufispace_s9502_16smt_lpc/mb_cpld/led_ctrl`
        ret=$?
        if [ $ret -eq 0 ]; then
        ...
        front_led_blink=`cat /sys/devices/platform/x86_64_ufispace_s9502_16smt_lpc/mb_cpld/led_clear`
        ret=$?
        if [ $ret -eq 0 ]; then
