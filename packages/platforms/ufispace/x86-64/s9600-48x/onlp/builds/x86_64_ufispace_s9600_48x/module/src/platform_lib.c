...
psu_present_get(int *pw_present, int id)
{
    ...
    if ((rc = file_read_hex(&status, "/sys/bus/i2c/devices/1-0030/cpld_psu_status")) != ONLP_STATUS_OK) {
        return ONLP_STATUS_E_INTERNAL;
    }
...
sysi_platform_info_get(onlp_platform_info_t* pi)
{
    ...
    int mb_cpld1_addr = 0x701, mb_cpld1_board_type_rev, mb_cpld1_hw_rev, mb_cpld1_build_rev;
    int i;
    char bios_out[32];
