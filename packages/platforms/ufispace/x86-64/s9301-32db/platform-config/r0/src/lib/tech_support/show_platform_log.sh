...
SYSFS_LPC="/sys/devices/platform/x86_64_ufispace_s9301_32db_lpc"
...
function _is_sfp_no_abs {
    ...
    # no present pin before beta build
    if [ $HW_REV_ID -lt 2 ]; then
        eval "$1=1" 
    else
        eval "$1=0"
    fi
...
function _show_sfp_port_status_sysfs {
    ...
    if [[ "${MODEL_NAME}" == *"S9301-32DB"* ]]; then
        ...
        do
            ...
            if [[ ${no_abs} -eq 1 && ($i == 0 || $i == 1) ]]; then
                # sfp 0/1 need to use ethtool to check if abs
                # get netif name according NOS
                _get_netif_name $i
                ret=`ethtool -m $NET_IF raw on length 1 2>/dev/null`
                if [[ $? -eq 0 ]]; then
                    port_absent="0"
                else
                    port_absent="1"
            ...
            if [ "${port_absent}" == "0" ]; then
                ...
                if [[ $i == 0 || $i == 1 ]]; then
                    ...
                    ethtool -m $NET_IF raw on length 256 > $output_file 2>/dev/null
                    sysfs=$output_file
                else
...
function _show_qsfp56_port_status_sysfs {
    _banner "Show QSFP56 Port Status / EEPROM"
    echo "    Show QSFP56 Port Status / EEPROM, please wait..."
    ...
    if [[ "${MODEL_NAME}" == *"S9301-32DB"* ]]; then
        ...
        for (( i=0; i<${#port_status_bit_idx_array[@]}; i++ ))
        do
            # Module QSFP56 Port Interrupt Status (0: Interrupted, 1:No Interrupt)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfp56_mod_int_${port_status_sysfs_idx_array[${i}]}"
            _check_filepath $sysfs
            ...
            # QSFP56 Port Fuse Interrupt Status (0: Interrupted, 1:No Interrupt)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfp56_fuse_int_${port_status_sysfs_idx_array[${i}]}"
            _check_filepath $sysfs
            ...
            # Module QSFP56 Port Absent Status (0: Present, 1:Absence)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfp56_pres_${port_status_sysfs_idx_array[${i}]}"
            _check_filepath $sysfs
            ...
            # Module QSFP56 Port Get Low Power Mode Status (0: Normal Power Mode, 1:Low Power Mode)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfp56_lp_mode_${port_status_sysfs_idx_array[${i}]}"
            _check_filepath $sysfs
            ...
            # Module QSFP56 Port Reset Status (0:Reset, 1:Normal)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfp56_reset_ctrl_${port_status_sysfs_idx_array[${i}]}"
            _check_filepath $sysfs
            ...
            # Module QSFP56 Port Dump EEPROM
            ...
            _echo "[QSFP56 Port${i} Module Interrupt Reg Raw]: ${port_module_interrupt_reg}"
            _echo "[QSFP56 Port${i} Module Interrupt]: ${port_module_interrupt}"
            _echo "[QSFP56 Port${i} Fuse Interrupt Reg Raw]: ${port_fuse_interrupt_reg}"
            _echo "[QSFP56 Port${i} Fuse Interrupt]: ${port_fuse_interrupt}"
            _echo "[QSFP56 Port${i} Module Absent Reg Raw]: ${port_module_absent_reg}"
            _echo "[QSFP56 Port${i} Module Absent ]: ${port_module_absent}"
            _echo "[QSFP56 Port${i} Low Power Mode Reg Raw]: ${port_lp_mode_reg}"
            _echo "[QSFP56 Port${i} Low Power Mode]: ${port_lp_mode}"
            _echo "[QSFP56 Port${i} Reset Reg Raw]: ${port_reset_reg}"
            _echo "[QSFP56 Port${i} Reset Status]: ${port_reset}"
...
function _show_qsfp56_port_status {
    if [ "${BSP_INIT_FLAG}" == "1" ]; then
        _show_qsfp56_port_status_sysfs
    fi
}
...
function _show_qsfpdd_port_status_sysfs {
    ...
    if [[ "${MODEL_NAME}" == *"S9301-32DB"* ]]; then
        bus_id="2"
        qsfpdd_port_eeprom_bus_id_base=41
        port_status_cpld_addr="0031"                                                     
        ...
        do
            ...
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfpdd_mod_int"
            _check_filepath $sysfs
            port_module_interrupt_reg=$(eval "cat $sysfs ${LOG_REDIRECT}")
            port_module_interrupt=$(( (port_module_interrupt_reg & 1 << ${port_status_bit_idx_array[i]}) >> ${port_status_bit_idx_array[i]} ))

            # QSFPDD Port Fuse Interrupt Status (0: Interrupted, 1:No Interrupt)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfpdd_fuse_int"
            _check_filepath $sysfs
            port_fuse_interrupt_reg=$(eval "cat $sysfs ${LOG_REDIRECT}")
            port_fuse_interrupt=$(( (port_fuse_interrupt_reg & 1 << ${port_status_bit_idx_array[i]}) >> ${port_status_bit_idx_array[i]} ))
            
            # Module QSFPDD Port Absent Status (0: Present, 1:Absence)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfpdd_pres"
            _check_filepath $sysfs
            port_module_absent_reg=$(eval "cat $sysfs ${LOG_REDIRECT}")
            port_module_absent=$(( (port_module_absent_reg & 1 << ${port_status_bit_idx_array[i]}) >> ${port_status_bit_idx_array[i]} ))

            # Module QSFPDD Port Get Low Power Mode Status (0: Normal Power Mode, 1:Low Power Mode)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfpdd_lp_mode"
            _check_filepath $sysfs
            port_lp_mode_reg=$(eval "cat $sysfs ${LOG_REDIRECT}")
            port_lp_mode=$(( (port_lp_mode_reg & 1 << ${port_status_bit_idx_array[i]}) >> ${port_status_bit_idx_array[i]} ))
            
            # Module QSFPDD Port Reset Status (0:Reset, 1:Normal)
            sysfs="/sys/bus/i2c/devices/${bus_id}-${port_status_cpld_addr}/cpld_qsfpdd_reset_ctrl"
            _check_filepath $sysfs
            port_reset_reg=$(eval "cat $sysfs ${LOG_REDIRECT}")
            port_reset=$(( (port_reset_reg & 1 << ${port_status_bit_idx_array[i]}) >> ${port_status_bit_idx_array[i]} ))
