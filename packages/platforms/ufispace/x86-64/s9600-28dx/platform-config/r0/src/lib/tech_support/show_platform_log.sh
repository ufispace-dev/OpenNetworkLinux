...
function _update_gpio_offset {
    ...
    if [ -z "${max_gpiochip_num}" ]; then
        GPIO_OFFSET=0
    elif [ ${max_gpiochip_num} -lt 228 ]; then
        GPIO_OFFSET=228
    else
        GPIO_OFFSET=0
    fi
...
function _show_board_info {
    ...
    model_id_array=($((2#00010101)))
    model_name_array=("S9600-28DX")
    model_name=""
...
function _cpld_version_sysfs {
    ...
    cpu_cpld_version_h=`cat /sys/devices/platform/x86_64_ufispace_s9600_28dx_lpc/cpu_cpld/cpu_cpld_version_h`
    ret=$?
    if [ $ret -eq 0 ]; then
...
function _show_rov_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-28DX"* ]]; then
        ...
        rov_config_reg="0x21"
        rov_output_reg="0x8b"
        rov_name_array=("cpld_mac0_synce_rov")
...
function _show_cpld_interrupt_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-28DX"* ]]; then
        ...
        cpld_qsfp_addr_array=("0031" "0031" "0032")
        cpld_qsfpdd_addr_array=("0032")
        sysfs_array=("cpld_qsfp_intr_port" "cpld_qsfp_intr_present")
		sysfs_index_array=("0_7" "8_15" "16_23")
