...
const char sysfs_attr_suffix[][8] = {"0_7", "8_15", "16_23", "0_3"};
...
static int ufi_port_to_cpld_addr(int port)
{
    ...
    } else if (port >= 16 && port < QSFP_NUM) {
        cpld_addr = CPLD_BASE_ADDR[2];
    } else if (port >= QSFP_NUM && port < QSFPX_NUM) {
        cpld_addr = CPLD_BASE_ADDR[2];
    } else if (IS_SFP(port)) {
        cpld_addr = CPLD_BASE_ADDR[1];
    } else {
