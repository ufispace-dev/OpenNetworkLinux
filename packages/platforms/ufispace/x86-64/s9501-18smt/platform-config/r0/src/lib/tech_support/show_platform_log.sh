...
function _check_bsp_init {
    ...
    if [ -f /sys/devices/platform/x86_64_ufispace_s9501_*_lpc/bsp/bsp_version ]; then
        BSP_INIT_FLAG=1
    else
        BSP_INIT_FLAG=0
    fi
...
function _show_board_info {
    ...
    ext_sku_name_array=("28SMT" "18SMT" "16SMT")
    deph_name_array=("NPI" "GA")
    model_id_array=($((2#00000011)))
    model_name_array=("S9501")
...
function _cpld_version_sysfs {
    ...
    if [ "${SKU_NAME}" == "28SMT" ]; then
        if _check_filepath "/sys/devices/platform/x86_64_ufispace_s9501_28smt_lpc/mb_cpld/mb_cpld_1_version"; then
            mb_cpld_ver=$(eval "cat /sys/devices/platform/x86_64_ufispace_s9501_28smt_lpc/mb_cpld/mb_cpld_1_version ${LOG_REDIRECT}")
            _echo "[MB CPLD Version]: $(( (mb_cpld_ver & 2#11000000) >> 6)).$(( mb_cpld_ver & 2#00111111 ))"
        fi
    elif [ "${SKU_NAME}" == "18SMT" ]; then
        if _check_filepath "/sys/devices/platform/x86_64_ufispace_s9501_18smt_lpc/mb_cpld/mb_cpld_1_version"; then
            mb_cpld_ver=$(eval "cat /sys/devices/platform/x86_64_ufispace_s9501_18smt_lpc/mb_cpld/mb_cpld_1_version ${LOG_REDIRECT}")
            _echo "[MB CPLD Version]: $(( (mb_cpld_ver & 2#11000000) >> 6)).$(( mb_cpld_ver & 2#00111111 ))"
        fi
    elif [ "${SKU_NAME}" == "16SMT" ]; then
        if _check_filepath "/sys/devices/platform/x86_64_ufispace_s9501_16smt_lpc/mb_cpld/mb_cpld_1_version"; then
            mb_cpld_ver=$(eval "cat /sys/devices/platform/x86_64_ufispace_s9501_16smt_lpc/mb_cpld/mb_cpld_1_version ${LOG_REDIRECT}")
            _echo "[MB CPLD Version]: $(( (mb_cpld_ver & 2#11000000) >> 6)).$(( mb_cpld_ver & 2#00111111 ))"
        fi
    else
        _echo "Unknown MODEL_NAME (${MODEL_NAME}), exit!!!"
...
function _show_system_led_sysfs {
    ...
    if [ "${MODEL_NAME}" == "S9501" ]; then
        front_led_color=`${IOGET} 0x735`
        ret=$?
        if [ $ret -eq 0 ]; then
            front_led_color=`echo ${front_led_color} | awk -F" " '{print $NF}'`
            front_led_color=$((front_led_color))
        else
            _echo "Get front led color failed ($ret), Exit!!"
            exit $ret
        fi

        front_led_ctrl=`${IOGET} 0x736`
        ret=$?
        if [ $ret -eq 0 ]; then
        ...
        # System LED
        sys_color=$(((front_led_color & 2#00010000) >>4)) # (1: green, 0: yellow)
