...
#define SYS_FAN_UPPER_MAX_RPM 16300
#define SYS_FAN_LOWER_MAX_RPM  15500
...
onlp_fan_info_t fan_info[] = {
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_0_U),
            .description = "CHASSIS FAN 0 UPPER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_0_L),
            .description = "CHASSIS FAN 0 LOWER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_1_U),
            .description = "CHASSIS FAN 1 UPPER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_1_L),
            .description = "CHASSIS FAN 1 LOWER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_2_U),
            .description = "CHASSIS FAN 2 UPPER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_2_L),
            .description = "CHASSIS FAN 2 LOWER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_3_U),
            .description = "CHASSIS FAN 3 UPPER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_3_L),
            .description = "CHASSIS FAN 3 LOWER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_4_U),
            .description = "CHASSIS FAN 4 UPPER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
    ...
    {
        .hdr = {
            .id = ONLP_FAN_ID_CREATE(ONLP_FAN_4_L),
            .description = "CHASSIS FAN 4 LOWER",
            .poid = POID_0,
        },
        .status = FAN_STATUS,
        .caps = FAN_CAPS,
...
static int get_node(int id, fan_node_t *node)
{
    board_t board = {0};
    if(node == NULL) {
    ...
    switch(node->local_id) {
        case ONLP_FAN_0_U:
            node->present = BMC_ATTR_ID_FAN0_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN0_RPM_UPPER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_0;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_UPPER_MAX_RPM;
            node->presence_path = SYSFS_FAN0_ABS;
            break;
        case ONLP_FAN_0_L:
            node->present = BMC_ATTR_ID_FAN0_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN0_RPM_LOWER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_0;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_LOWER_MAX_RPM;
            node->presence_path = SYSFS_FAN0_ABS;
            break;
        case ONLP_FAN_1_U:
            node->present = BMC_ATTR_ID_FAN1_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN1_RPM_UPPER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_1;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_UPPER_MAX_RPM;
            node->presence_path = SYSFS_FAN1_ABS;
            break;
        case ONLP_FAN_1_L:
            node->present = BMC_ATTR_ID_FAN1_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN1_RPM_LOWER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_1;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_LOWER_MAX_RPM;
            ...
            break;
        case ONLP_FAN_2_U:
            node->present = BMC_ATTR_ID_FAN2_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN2_RPM_UPPER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_2;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_UPPER_MAX_RPM;
            node->presence_path = SYSFS_FAN2_ABS;
            break;
        case ONLP_FAN_2_L:
            node->present = BMC_ATTR_ID_FAN2_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN2_RPM_LOWER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_2;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_LOWER_MAX_RPM;
            ...
            break;
        case ONLP_FAN_3_U:
            node->present = BMC_ATTR_ID_FAN3_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN3_RPM_UPPER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_3;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_UPPER_MAX_RPM;
            node->presence_path = SYSFS_FAN3_ABS;
            break;
        case ONLP_FAN_3_L:
            node->present = BMC_ATTR_ID_FAN3_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN3_RPM_LOWER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_3;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_LOWER_MAX_RPM;
            ...
            break;
        case ONLP_FAN_4_U:
            node->present = BMC_ATTR_ID_FAN4_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN4_RPM_UPPER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_4;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_UPPER_MAX_RPM;
            node->presence_path = SYSFS_FAN4_ABS;
            break;
        case ONLP_FAN_4_L:
            node->present = BMC_ATTR_ID_FAN4_PRSNT_L;
            node->rpm = BMC_ATTR_ID_FAN4_RPM_LOWER;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_FAN_TRAY_4;
            node->type = TYPE_FANTRAY;
            node->max_rpm1 = SYS_FAN_LOWER_MAX_RPM;
            ...
            break;
        case ONLP_PSU_0_FAN:
            node->present = BMC_ATTR_ID_PSU0_PRSNT_L;
            node->rpm = BMC_ATTR_ID_PSU0_FAN;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_ONLP_PSU_0;
            node->type = TYPE_PSU;
            node->parent = ONLP_PSU_ID_CREATE(ONLP_PSU_0);
            node->max_rpm1 = PSU_FAN_RPM_MAX_AC;
            node->max_rpm2 = PSU_FAN_RPM_MAX_DC;
            break;
        case ONLP_PSU_1_FAN:
            node->present = BMC_ATTR_ID_PSU1_PRSNT_L;
            node->rpm = BMC_ATTR_ID_PSU1_FAN;
            node->dir = BMC_OEM_IDX_INVALID;
            node->fru_id = BMC_FRU_IDX_ONLP_PSU_0;
            node->type = TYPE_PSU;
            node->parent = ONLP_PSU_ID_CREATE(ONLP_PSU_1);
            node->max_rpm1 = PSU_FAN_RPM_MAX_AC;
            node->max_rpm2 = PSU_FAN_RPM_MAX_DC;
            break;
        default:
...
/**
 * @brief Gets the fan presence status by reading its CPLD sysfs attribute.
 * @param node [in] The fan node structure containing the sysfs path.
 * @param presence [out] Pointer to store the result (1 if present, 0 if not).
 * @return ONLP_STATUS_OK on success, or an error code.
 */
static int get_fan_present_status(const fan_node_t* node, int* presence)
{
    int absent = 1; // Default to absent

    /* Input validation */
    if (node == NULL || presence == NULL) {
        return ONLP_STATUS_E_INTERNAL;
    }

    // Set default return value to not present
    *presence = 0;

    // Check if a sysfs path is defined for this fan
    if (node->presence_path == NULL) {
        return ONLP_STATUS_OK;
    }

    if (onlp_file_read_int(&absent, node->presence_path) == ONLP_STATUS_OK) {
        *presence = !absent;
    }
    else {
        AIM_LOG_ERROR("Failed to read fan presence from %s", node->presence_path);
    }
 ...
 */
static int update_fru_info(int id, onlp_fan_info_t* info)
{
    bmc_fru_t fru = {0};
    fan_node_t node = {0};
 ...
 */
static int get_bmc_fan_info(int id, onlp_fan_info_t* info)
{
    ...
    ONLP_TRY(get_node(id, &node));
    *info = fan_info[node.local_id];
    /* present */
    if(IS_FANTRAY(node)) {
        ONLP_TRY(get_fan_present_status(&node, &presence));
        if (presence) {
    ...
    if(0) {
        ...
        if(info->status & ONLP_FAN_STATUS_PRESENT) {
            ...
            if(dir == BMC_FAN_DIR_B2F) {
                ...
                info->status &= ~ONLP_FAN_STATUS_F2B;
                info->status &= ~ONLP_FAN_STATUS_B2F;
                AIM_LOG_ERROR("unknown dir fan id (%d), func=%s", node.local_id, __FUNCTION__);
            }
        } else {
            info->status &= ~ONLP_FAN_STATUS_F2B;
            info->status &= ~ONLP_FAN_STATUS_B2F;
    ...
    if(info->status & ONLP_FAN_STATUS_PRESENT) {
        ...
        if(IS_FANTRAY(node)) {
                ...
                ONLP_TRY(get_psu_support_info(node.parent, &psu_support_info, NULL));
                psu_type = psu_support_info.type;
        ...
        ONLP_TRY(update_fru_info(id, info));
    }
    return ONLP_STATUS_OK;
}
...
{
    ONLP_TRY(get_bmc_fan_info(id, rv));
    return ONLP_STATUS_OK;
}
...
int onlp_fani_status_get(onlp_oid_t id, uint32_t* rv)
{
    ...
    ONLP_TRY(get_bmc_fan_info(id, &info));
    *rv = info.status;
...
int onlp_fani_hdr_get(onlp_oid_t id, onlp_oid_hdr_t* hdr)
{
    ...
    ONLP_TRY(get_node(id, &node));
    *hdr = fan_info[node.local_id].hdr;
