...
/**
 * @brief Update device class for QSFPX ports
 *
 * This function updates the device class for a given QSFPX port.
...
int ufi_reorg_dev_class(int port)
{
    ...
    /* This is a wrapper for backward compatibility. It now calls the new core logic.*/
    return onlp_sfpi_dev_class_update_port(port);
}
#endif
...
int onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
    ...
    bus = ufi_port_to_eeprom_bus(port);
#if REORG_DEV_CLASS_ENABLE
    /* The ufi_reorg_dev_class has its own port type check. */
    ...
    if((rc = onlp_file_read(data, 256, &size, "%s", sysfs_path)) < 0) {
    ...
    *rv = 0;
    ...
    switch (control) {
        ...
        case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
            if (IS_SFP(port) || IS_SFPDD(port) || IS_QSFPX(port)) {
                *rv = 1;
            }
            break;
        default:
        ...
        case ONLP_SFP_CONTROL_RESET:
            {
                if (IS_QSFP(port)) {
                    ...
                    if ((rc=onlp_file_write_int(reg_val, SYS_FMT_OFFSET, cpld_bus, cpld_addr, SYSFS_QSFP_RESET, attr_offset)) < 0) {
                        ...
                        AIM_LOG_ERROR(SYS_FMT_OFFSET, cpld_bus, cpld_addr, SYSFS_QSFP_RESET, attr_offset);
                        check_and_do_i2c_mux_reset(port);
                        return ONLP_STATUS_E_INTERNAL;
                    }
                    rc = ONLP_STATUS_OK;
        ...
        case ONLP_SFP_CONTROL_TX_DISABLE_CHANNEL:
            {
                if (IS_SFP(port) || IS_SFPDD(port)) {
                    ...
                    dev_class = onlp_sfpi_dev_class_update(port);
                    if (dev_class <= 0) {
        ...
        case ONLP_SFP_CONTROL_LP_MODE:
            {
                if (IS_QSFP(port)) {
                    ...
                    if (onlp_file_write_int(reg_val, SYS_FMT_OFFSET, cpld_bus, cpld_addr, SYSFS_QSFP_LPMODE, attr_offset) < 0) {
                        ...
                        AIM_LOG_ERROR(SYS_FMT_OFFSET, cpld_bus, cpld_addr, SYSFS_QSFP_LPMODE, attr_offset);
                        check_and_do_i2c_mux_reset(port);
                        return ONLP_STATUS_E_INTERNAL;
                    }
                    rc = ONLP_STATUS_OK;
                    ...
                    if (onlp_file_write_int(reg_val, SYS_FMT, cpld_bus, cpld_addr, SYSFS_QSFPDD_LPMODE) < 0) {
                        ...
                        AIM_LOG_ERROR(SYS_FMT, cpld_bus, cpld_addr, SYSFS_QSFPDD_LPMODE);
                        check_and_do_i2c_mux_reset(port);
                        return ONLP_STATUS_E_INTERNAL;
                    }
                    rc = ONLP_STATUS_OK;
