...
/*
 * The HAVE_DEVLINK_PARAMS will be enabled in kernel versions 4.19 and above.
 * However, if the kernel configuration CONFIG_NET_DEVLINK is not enabled,
 * the code inside this block will still attempt to execute the ice_devlink_tc_params_register
 * and ice_devlink_tc_params_unregister APIs, resulting in an unknown symbol error.
 * Note that these two APIs will not be built when CONFIG_NET_DEVLINK is not enabled.
 * To avoid this issue, we need to add a check using IS_ENABLED(CONFIG_NET_DEVLINK).
 */
