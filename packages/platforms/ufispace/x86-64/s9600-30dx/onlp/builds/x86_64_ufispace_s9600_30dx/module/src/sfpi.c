...
int onlp_sfpi_eeprom_read(int port, uint8_t data[256])
{
    ...
    if (IS_QSFPX(port)) {
        ...
        if((rc = onlp_file_read(data, expect_size, &size, sysfs_path)) < 0) {
        ...
        snprintf(sysfs_path, sizeof(sysfs_path), "/tmp/.sfp.%s.eeprom", if_name);
        ...
        ONLP_TRY(onlp_file_read(data, expect_size, &ret_size, sysfs_path));
    } else {
        return ONLP_STATUS_E_PARAM;
    }
