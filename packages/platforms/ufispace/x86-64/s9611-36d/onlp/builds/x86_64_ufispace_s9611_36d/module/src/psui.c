...
static onlp_psu_info_t __onlp_psu_info[] = {
    { }, /* Not used */
    {
        ...
        .model = "",
        .serial = "",
        .caps = ONLP_PSU_CAPS_GET_VIN|ONLP_PSU_CAPS_GET_VOUT|ONLP_PSU_CAPS_GET_IIN|ONLP_PSU_CAPS_GET_IOUT,
...
int get_psu_present_status(int local_id, int *psu_present)
{
    ...
    *psu_present = (psu_reg_value & mask) ? 0 : 1;
...
static int get_fru_id(int local_id)
{
    switch (local_id) {
        case ONLP_PSU_0:
            return BMC_FRU_IDX_ONLP_PSU_0;
        case ONLP_PSU_1:
            return BMC_FRU_IDX_ONLP_PSU_1;
        default:
...
int get_psu_type(int local_id, int *psu_type, bmc_fru_t *fru_in)
{
    ...
    if(fru_in == NULL) {
        fru = &fru_tmp;
        ONLP_TRY(bmc_fru_read(get_fru_id(local_id), fru));
    } else {
        fru = fru_in;
    }
