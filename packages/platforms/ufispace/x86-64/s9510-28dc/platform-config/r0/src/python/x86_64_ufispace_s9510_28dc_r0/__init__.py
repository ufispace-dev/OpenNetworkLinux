...
class OnlPlatform_x86_64_ufispace_s9510_28dc_r0(OnlPlatformUfiSpace):
    ...
    def init_gpio(self, gpio_max):
        ...
        for off in list(range(-20, -24, -1)) + \
                 list(range(-32, -48, -1)) + \
                 list(range(-56, -64, -1)) + \
                 list(range(-116, -120, -1)):
            gpio_dir[gpio_max + off] = "low"

        # init GPIO direction to output high
        for off in list(range(-16, -20, -1)) + \
                 list(range(-96, -112, -1)) + \
    ...
    def baseconfig(self):
        ...
        try:
            output = subprocess.check_output(["cat", "/sys/devices/platform/x86_64_ufispace_s9510_28dc_lpc/mb_cpld/board_hw_id"])
        except Exception as e:
        ...
        # get gpio max
        try:
            output = subprocess.check_output(["cat", "/sys/devices/platform/x86_64_ufispace_s9510_28dc_lpc/bsp/bsp_gpio_max"])
        except Exception as e:
