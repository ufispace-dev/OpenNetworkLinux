...
SYSFS_LPC="/sys/bus/platform/devices/x86_64_ufispace_s9601_102xc_lpc"
...
function _update_gpio_max {
    ...
    GPIO_MAX=$(cat /sys/devices/platform/x86_64_ufispace_s9601_102xc_lpc/bsp/bsp_gpio_max)
    ...
    GPIO_BASE=$(cat /sys/devices/platform/x86_64_ufispace_s9601_102xc_lpc/bsp/bsp_gpio_base)
...
function _check_bsp_init {
    ...
    if [ -f "/sys/devices/platform/x86_64_ufispace_s9601_102xc_lpc/bsp/bsp_gpio_max" ]; then
        BSP_INIT_FLAG=1
    else
        BSP_INIT_FLAG=0
    fi
    ...
    case $MODEL_NAME in
        *$PLAT* )
            ...
            if _check_dirpath "$SYSFS_CPLD5"; then
                reg_dump=$(eval "i2cdump -f -y 1 0x34 ${LOG_REDIRECT}")
