...
void check_and_do_i2c_mux_reset(int port)
{
    ...
    if(access("/sys/bus/i2c/devices/1-0030/cpld_id", F_OK) != -1 ) {
        ...
        if (ret != 0) {
            if(access("/sys/devices/platform/x86_64_ufispace_s9700_23d_lpc/cpu_cpld/mux_reset", F_OK) != -1 ) {
                //AIM_LOG_SYSLOG_WARN("I2C bus is stuck!! (port=%d)\r\n", port);
                snprintf(cmd_buf, sizeof(cmd_buf), "echo 0 > /sys/devices/platform/x86_64_ufispace_s9700_23d_lpc/cpu_cpld/mux_reset 2> /dev/null");
                ret = system(cmd_buf);
