...
function _update_gpio_max {
    ...
    local sysfs="/sys/devices/platform/x86_64_ufispace_s9110_32x_lpc/bsp/bsp_gpio_max"
...
function _get_i2c_root {
	...
	QSFP_BUS_BASE=$((2+8*2))
	SFP_BUS_BASE=$((QSFP_BUS_BASE + 8*4 + 1))
}

function _init_sysfs {    
    SYSFS_CPLD1="${SYSFS_DEV}/${ismt_bus}-0030"
    SYSFS_CPLD2="${SYSFS_DEV}/${ismt_bus}-0031"
}
...
function _show_qsfp_status_sysfs {
    ...
    local sysfs_idx_array=("0_7" "8_15" "16_23" "24_31")
	local port_start=0
    ...
    do
        ...
        # Show Port Status	   
        _echo "[Port${i} Module INT (L)]: ${port_intr_l}"
		_echo "[Port${i} Module Absent ]: ${port_absent_l}"		
        _echo "[Port${i} Low Power Mode]: ${port_lp_mode}"
		_echo "[Port${i} Reset Status  ]: ${port_reset_l}"		
		_echo "[Port${i} Status Reg Raw       ]: ${port_intr_l_reg}"        
		_echo "[Port${i} Module Absent Reg Raw]: ${port_absent_l_reg}"        
        _echo "[Port${i} Config Reg Raw       ]: ${port_lp_mode_reg}"        
        _echo "[Port${i} Reset Reg Raw        ]: ${port_reset_l_reg}"
        ...
        eeprom_path="/sys/bus/i2c/devices/$((QSFP_BUS_BASE + i))-0050/eeprom"			
