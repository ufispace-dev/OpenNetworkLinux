...
# Sysfs
SYSFS_LPC="/sys/devices/platform/x86_64_ufispace_s9600_56dx_lpc"
...
function _show_board_info {
    ...
    model_id_array=($((2#00010100)))
    model_name_array=("S9600-56DX")
    model_name=""
...
function _cpld_version_sysfs {
    ...
    cpu_cpld_version_h=`cat /sys/devices/platform/x86_64_ufispace_s9600_56dx_lpc/cpu_cpld/cpu_cpld_version_h`
    ret=$?
    if [ $ret -eq 0 ]; then
...
function _show_rov_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-56DX"* ]]; then
        bus_id="1"
        rov_i2c_bus="9"
        rov_i2c_addr=("0x3e" "0x3c")
        rov_mask=(2#00000111 2#00000111)
        rov_shift=(0 0)
        rov_config_reg="0x21"
        rov_output_reg="0x8b"
        rov_name_array=("cpld_mac0_synce_rov" "cpld_mac1_synce_rov")

        # Read MAC ROV Status
        _check_filepath "/sys/bus/i2c/devices/${bus_id}-0030/${rov_name_array[0]}"
        cpld_mac_rov_reg[0]=$(eval "cat /sys/bus/i2c/devices/${bus_id}-0030/${rov_name_array[0]} ${LOG_REDIRECT}")
        _check_filepath "/sys/bus/i2c/devices/${bus_id}-0030/${rov_name_array[1]}"
        cpld_mac_rov_reg[1]=$(eval "cat /sys/bus/i2c/devices/${bus_id}-0030/${rov_name_array[1]} ${LOG_REDIRECT}")

        if [ -c "/dev/i2c-${rov_i2c_bus}" ]; then

            for (( i=0; i<${#rov_i2c_addr[@]}; i++ ))
            do
                mac_rov_stamp=$(((cpld_mac_rov_reg[i] & rov_mask[i]) >> rov_shift[i]))
                ...
                i2cset -y ${rov_i2c_bus} ${rov_i2c_addr[i]} 0x0 0x3f
                i2cset -y ${rov_i2c_bus} ${rov_i2c_addr[i]} 0x27 0xb37c w
                i2cset -y ${rov_i2c_bus} ${rov_i2c_addr[i]} 0x0 0x0
                rov_controller_output=`i2cget -y ${rov_i2c_bus} ${rov_i2c_addr[i]} ${rov_output_reg} w`
...
function _show_nif_port_status_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-56DX"* ]]; then
        ...
        port_status_sysfs_idx_array=("0_7" "0_7" "0_7" "0_7" "0_7" \
                                     ...
                                     "16_23" "16_23" "16_23" "16_23" "24_31" \
                                     "24_31" "24_31" "24_31" "24_31" "24_31" \
                                     "24_31" "24_31" "32_39" "32_39" "32_39" \
                                     "32_39" "32_39" "32_39" "32_39" "32_39" \
                                     "40_47" "40_47" "40_47" "40_47" "40_47" \
                                     "40_47" "40_47" "40_47" "0_3" "0_3" \
                                     "0_3" "0_3" "4_7" "4_7" "4_7" \
                                     "4_7")
...
function _show_cpld_interrupt_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-56DX"* ]]; then
        ...
        cpld_qsfp_addr_array=("0031" "0031" "0032" "0033" "0034" "0034")
        cpld_qsfpdd_addr_array=("0032" "0033")
        sysfs_array=("cpld_qsfp_intr_port" "cpld_qsfp_intr_present")
		sysfs_index_array=("0_7" "8_15" "16_23" "24_31" "32_39" "40_47")
        sysfs_desc_array=("QSFP Port" "QSFP Present")
        ...
        do
            ...
            do
                cpld_interrupt_reg=$(eval "cat /sys/bus/i2c/devices/${bus_id[1]}-${cpld_qsfp_addr_array[j]}/${sysfs_array[${i}]}_${sysfs_index_array[${j}]} ${LOG_REDIRECT}")
        ...
        sysfs_array=("cpld_qsfpdd_intr_port" "cpld_qsfpdd_intr_present")
		sysfs_index_array=("0_3" "4_7")
        sysfs_desc_array=("QSFPDD Port" "QSFPDD Present")
        ...
        do
            for (( j=0; j<${#sysfs_index_array[@]}; j++ ))
            do
                cpld_interrupt_reg=$(eval "cat /sys/bus/i2c/devices/${bus_id[2]}-${cpld_qsfpdd_addr_array[j]}/${sysfs_array[${i}]}_${sysfs_index_array[${j}]} ${LOG_REDIRECT}")
...
function _show_system_led_sysfs {
    ...
    if [[ $MODEL_NAME == *"S9600-56DX"* ]]; then
		...
		_check_filepath "/sys/bus/i2c/devices/1-0030/cpld_system_led_loc"
		...
		system_led_id=$(eval "cat /sys/bus/i2c/devices/1-0030/cpld_system_led_loc ${LOG_REDIRECT}")
		...
		_echo "[System LED LOC]: ${system_led_id}"
