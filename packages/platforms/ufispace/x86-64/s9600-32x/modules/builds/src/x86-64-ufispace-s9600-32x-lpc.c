/*
 * A lpc driver for the ufispace_s9600_32x
 *
...
#define DRIVER_NAME "x86_64_ufispace_s9600_32x_lpc"
#define CPU_BDE 0
#define CPU_SKY 1
...
#define REG_MB_CPLD_BUILD                 (REG_BASE_MB + 0x80)
#define REG_MB_RESET_CTRL_4               (REG_BASE_MB + 0x24)
#define REG_MB_RESET_CTRL_5               (REG_BASE_MB + 0x25)
#define REG_MB_MUX_CTRL                   (REG_BASE_MB + 0x16)
        ...
        struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (mux_reset_flag == 0) {
        if (val == 0) {
            ...
            //reset mux on QSFP ports
            mux_reset_1_reg_val = inb(REG_MB_RESET_CTRL_5);
            outb((mux_reset_1_reg_val & 0b00000000), REG_MB_RESET_CTRL_5);
            mdelay(100);

            //reset mux on root mux
            mux_reset_2_reg_val = inb(REG_MB_RESET_CTRL_4);
            outb((mux_reset_2_reg_val & 0b11101000), REG_MB_RESET_CTRL_4);
            mdelay(100);

            //unset mux on QSFP ports
            outb((mux_reset_1_reg_val | 0b11111111), REG_MB_RESET_CTRL_5);
            mdelay(100);
            //unset mux on root mux
            outb((mux_reset_2_reg_val | 0b00010111), REG_MB_RESET_CTRL_4);
...
MODULE_DESCRIPTION("x86_64_ufispace_s9600_32x_lpc driver");
MODULE_LICENSE("GPL");
