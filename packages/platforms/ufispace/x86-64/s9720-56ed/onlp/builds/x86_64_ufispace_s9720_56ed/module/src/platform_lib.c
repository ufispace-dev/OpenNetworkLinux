...
bmc_info_t bmc_cache[] =
{
    ...
    [BMC_ATTR_ID_TEMP_ENV_TOP0]    = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_ENV_TOP0     , BMC_DATA_FLOAT , 0},            
    [BMC_ATTR_ID_TEMP_ENV_TOP1]    = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_ENV_TOP1     , BMC_DATA_FLOAT , 0},            
    ...
    [BMC_ATTR_ID_TEMP_ENV_MAC1]    = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_ENV_MAC1     , BMC_DATA_FLOAT , 0},  
    [BMC_ATTR_ID_TEMP_MAC0]        = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_MAC0         , BMC_DATA_FLOAT , 0},            
    [BMC_ATTR_ID_TEMP_MAC1]        = {HW_PLAT_ALL, BMC_ATTR_NAME_TEMP_MAC1         , BMC_DATA_FLOAT , 0},            
    ...
    [BMC_ATTR_ID_PSU0_PWROK_L]     = {HW_PLAT_ALL, BMC_ATTR_NAME_PSU0_PWROK_L      , BMC_DATA_BOOL  , 0},           
    [BMC_ATTR_ID_PSU1_PRSNT_L]     = {HW_PLAT_ALL, BMC_ATTR_NAME_PSU1_PRSNT_L      , BMC_DATA_BOOL  , 0},           
    [BMC_ATTR_ID_PSU1_PWROK_L]     = {HW_PLAT_ALL, BMC_ATTR_NAME_PSU1_PWROK_L      , BMC_DATA_BOOL  , 0},           
...
int get_board_version(board_t *board)
{
    ...
    //Get HW Version
    if(read_file_hex(&board->hw_rev   , LPC_MB_CPLD "cpld_hw_rev") != ONLP_STATUS_OK ||
       read_file_hex(&board->deph_id  , LPC_MB_CPLD "cpld_deph_rev") != ONLP_STATUS_OK ||
       read_file_hex(&board->hw_build , LPC_MB_CPLD "cpld_build_rev") != ONLP_STATUS_OK ||
       read_file_hex(&board->sku_id   , LPC_MB_CPLD "cpld_sku_id") != ONLP_STATUS_OK ||
       read_file_hex(&board->id_type  , LPC_MB_CPLD "cpld_brd_id_type") != ONLP_STATUS_OK)
    {
        board->hw_rev = 0;
        board->deph_id = 0;
        board->hw_build = 0;
...
int get_hw_rev_id(void)
{
    ...
    snprintf(hw_rev_cmd, sizeof(hw_rev_cmd), "cat " LPC_MB_CPLD "cpld_hw_rev");
...
int get_thermal_thld(int thermal_local_id,  temp_thld_t *temp_thld) {
    ...
    switch(thermal_local_id) {
        ...
        case ONLP_THERMAL_ENV0:
            temp_thld->warning = THERMAL_TEMP_ENV_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV_SHUTDOWN;
            return ONLP_STATUS_OK;
        ...
        case ONLP_THERMAL_ENV_EXT0:
            ...
            temp_thld->shutdown = THERMAL_TEMP_ENV_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV_TOP0:
            temp_thld->warning = THERMAL_TEMP_ENV_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV_SHUTDOWN;
            return ONLP_STATUS_OK;
        case ONLP_THERMAL_ENV_TOP1:
            temp_thld->warning = THERMAL_TEMP_ENV_WARNING;
            temp_thld->error = THERMAL_TEMP_ENV_ERROR;
            temp_thld->shutdown = THERMAL_TEMP_ENV_SHUTDOWN;
            return ONLP_STATUS_OK;
...
int get_gpio_max(int *gpio_max)
{
    ...
    if(read_file_hex(gpio_max, LPC_BSP_PATH "bsp_gpio_max") != ONLP_STATUS_OK)
    {
        *gpio_max = 511;
        rv = ONLP_STATUS_E_INVALID;
    }
