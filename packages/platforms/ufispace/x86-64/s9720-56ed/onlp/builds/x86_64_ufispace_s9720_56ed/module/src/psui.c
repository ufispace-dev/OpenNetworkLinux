...
static psu_support_info_t psu_support_list[] =
{
    ...
    {"DELTA", "DTQ3000AN12C12", ONLP_PSU_TYPE_DC48},
};
typedef struct
{
    int fru_id;
...
static const psu_attr_t psu_attr[] = {
    /*              fru_id                  abs_sysfs                pwrgd_sysfs */
    [ONLP_PSU_0] = {BMC_FRU_IDX_ONLP_PSU_0, SYSFS_CPLD1 "psu_0_prsnt", SYSFS_CPLD1 "psu_0_pg"},
    [ONLP_PSU_1] = {BMC_FRU_IDX_ONLP_PSU_1, SYSFS_CPLD1 "psu_1_prsnt", SYSFS_CPLD1 "psu_1_pg"},
};
...
int get_psu_present_status(int local_id, int *pw_present)
{
    ...
    sysfs = psu_attr[local_id].abs_sysfs;
...
static int get_psu_pwgood_status(int local_id, int *pw_good)
{
    ...
    sysfs = psu_attr[local_id].pwrgd_sysfs;
    ONLP_TRY(read_file_hex(&status, sysfs));
