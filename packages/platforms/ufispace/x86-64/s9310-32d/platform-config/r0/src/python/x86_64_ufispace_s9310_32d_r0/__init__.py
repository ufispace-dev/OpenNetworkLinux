...
class OnlPlatform_x86_64_ufispace_s9310_32d_r0(OnlPlatformUfiSpace):
    PLATFORM = 'x86-64-ufispace-s9310-32d-r0'
    MODEL = "s9310-32d"
    ...
    def check_i2c_status(self): 
        sysfs_mux_reset = "/sys/devices/platform/x86_64_ufispace_s9310_32d_lpc/mb_cpld/mux_reset"
    ...
    def baseconfig(self):
        ...
        # init CPLD
        self.insmod("x86-64-ufispace-s9310-32d-cpld")
        for i, addr in enumerate((0x30, 0x31, 0x32)):
            self.new_i2c_device("s9310_32d_cpld" + str(i+1), addr, 2)

        # config mac rov
        # done bye CPLD at power on          

        # onie syseeprom
        self.insmod("x86-64-ufispace-s9310-32d-onie-syseeprom.ko")
