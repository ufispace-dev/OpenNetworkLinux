...
SYSFS_CPLD1="${SYSFS_DEV}/5-0030"
SYSFS_CPLD2="${SYSFS_DEV}/5-0031"
SYSFS_CPLD3="${SYSFS_DEV}/5-0032"
SYSFS_CPLD4="${SYSFS_DEV}/5-0033"
SYSFS_CPLD5="${SYSFS_DEV}/5-0034"
...
function _update_gpio_max {
    ...
    GPIO_MAX=$(cat /sys/devices/platform/x86_64_ufispace_s9601_104bc_lpc/bsp/bsp_gpio_max)
    if [ $? -eq 1 ]; then
        GPIO_MAX_INIT_FLAG=0
    else
...
function _check_bsp_init {
    ...
    if [ -f "/sys/devices/platform/x86_64_ufispace_s9601_104bc_lpc/bsp/bsp_version" ]; then
        BSP_INIT_FLAG=1
    else
        BSP_INIT_FLAG=0
    fi
...
function _show_board_info {
    ...
    model_id_array=($((2#00101011)))
    model_name_array=("S9601-104BC")
    brd_id_def_id_array=(0 1 2 3) 
    brd_id_array=(
        "CSGR 4-bit/8-bit mode & DDC 8-bit mode" #0
        ""                                       #1
        "S9601-104BC and other new platform"     #2
...
function _cpld_version_i2c {
    ...
    if [[ $MODEL_NAME == "${PLAT}" ]]; then
        ...
        if [ ${ret} -eq ${TRUE} ]; then
            i2cset -y -f ${mux_i2c_bus} ${mux_i2c_addr} 0x8
...
function _show_i2c_tree_bus_mux_i2c {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        if [ "$ret" == "0" ]; then
            ...
            _show_i2c_mux_devices "${bus}" "${chip_addr1}" "8" "9548_ROOT_PORT_1-${chip_addr1}"
        ...
        if [ "$ret" == "0" ]; then
            ...
            local mux_name_array=("" "" "" \
                                  "9548_CHILD_QSFP_0_7" "9548_CHILD_SFP_64_71" \
...
function _show_i2c_device_info {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        pca954x_device_id=("0x73" "0x75" "0x72")
        pca954x_device_bus=("${ismt_bus}" "${i801_bus}" "${i801_bus}")
    else
        _echo "Unknown MODEL_NAME (${MODEL_NAME}), exit!!!"
        exit 1
    fi
...
function _show_sys_eeprom_sysfs {
    ...
    sys_eeprom=$(eval "cat /sys/bus/i2c/devices/4-0057/eeprom ${LOG_REDIRECT} | hexdump -C")
    _echo "[System EEPROM]:"
    _echo "${sys_eeprom}"
}
...
function _show_port_status_sysfs {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        sys_port_array=("${SYSFS_CPLD2}/cpld_sfp_port_0_7_pres"               #0
                         ...
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_103_pres"          #14
                         ...
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_103_rst"           #43
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_103_lpmode"        #44
                         ...
                         "${SYSFS_CPLD5}/cpld_qsfp_port_96_103_intr")         #61
...
function _show_system_led_sysfs {
    ...
    if [ "${MODEL_NAME}" == "${PLAT}" ]; then
        ...
        local led_sysfs_idx=( 2       1       3       0       0       4)
        local color=(         0       4       0       0       4      -1)
        local blink=(         2       6       2       2       6       2)
        local onoff=(         3       7       3       3       7       3)
        local freq=(          1       5       1       1       5       1)
