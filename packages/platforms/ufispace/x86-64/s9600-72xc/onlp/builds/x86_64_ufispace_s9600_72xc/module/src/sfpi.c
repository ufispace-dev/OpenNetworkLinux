...
#define MB_CPLD_QSFP_RESET_ATTR "cpld_qsfp_port_64_71_rst"
#define MB_CPLD_QSFP_LPMODE_ATTR "cpld_qsfp_port_64_71_lpmode"
...
port_type_info_t port_num_to_type(int port)
{
    ...
    else if ((port >= SFP_NUM) && (port < (SFP_NUM + QSFP_NUM)))
    { // QSFP
...
int sfp_control_set(port_type_info_t port_info, onlp_sfp_control_t control, int value)
{
    ...
    // write reg val back
    if ((rc = onlp_file_write_int(reg_val, sysfs)) != ONLP_STATUS_OK)
    {
