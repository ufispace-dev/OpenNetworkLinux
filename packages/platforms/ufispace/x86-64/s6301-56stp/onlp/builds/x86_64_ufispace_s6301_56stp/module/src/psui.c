...
{
    {"FSPGROUP", "YPEE0920EM-1A03P10", ONLP_PSU_TYPE_AC, ONLP_FAN_STATUS_F2B}
};
...
{
    uint8_t buffer[size], i2c_addr, reg;
    int ret, len, i2c_bus, pw_good;
    char cmd_buf[256] = {0};
    ...
    /* workaround to detect pus fw version register */
    reg = PSU_PMBUS_REVISION_0;
    snprintf(cmd_buf, sizeof(cmd_buf), "i2cget -y -f %d 0x%x 0x%x > /dev/null 2>&1", 
            i2c_bus, i2c_addr, reg);
    ret = system(cmd_buf);

    if(ret != 0) {
        // fail to access, use the other register to workaround
        reg = PSU_PMBUS_REVISION_1;
        workaround = true;
    }

    len = onlp_i2c_readb(i2c_bus, i2c_addr, reg, ONLP_I2C_F_FORCE);
    if(len < 0) {
        AIM_LOG_ERROR("Unable to read byte offset 0x%02x from pmbus %d-0x%02x)",
                        reg, i2c_bus, i2c_addr);
        return ONLP_STATUS_E_INTERNAL;
    }
    if(size < len) {
    ...
    /* read data */
    ret = onlp_i2c_block_read(i2c_bus, i2c_addr, reg, len, buffer,
                                ONLP_I2C_F_FORCE);
    if(workaround) {
        ret = onlp_i2c_block_read(i2c_bus, i2c_addr, reg, len, buffer,
                                    ONLP_I2C_F_FORCE);
